...
\section{Traditional rendering (rasterization)}
	...
	\subsection{Forward rendering}
		With forward rendering, for each object in the scene, we would render its fragments to the frame-buffer if they pass the z-test (if it is the current frontmost fragment). Since this is done for each object, it is likely that many fragments will overlap other fragments. However, we already computed all the lighting and shading for that fragment, so if it is not going to be visible anyway, than those computations were effectively wasted. This means that the more lights we add to the scene, the more lighting computations are going to be in vain.
	\subsection{Deferred rendering}
		The goal of deferred rendering is to defer lighting and shading calculations until after the frontmost fragments are determined. This is done by computing necessary geometry buffers (g-buffers) in a first pass that is called "geometry pass" and storing them in a texture-buffer instead of rendering to the screen. In the second pass (lighting pass) all lighting computations are performed and accumulated with the texture-buffer, which is then rendered to the screen. \\\\
	...
	\subsection{Geometry-buffer (G-buffer)}
		\begin{figure}[]
			\subfigure[A scene that was rendered using deferred lighting. The objects are locally illuminated by point-lights using g-buffers.]{\includegraphics[width=.24\textwidth]{img/wiki_deferred_shading.png}}%
			\subfigure[Example of an albedo-buffer. It shows a scene containing a torus, a sphere, a cuboid and a cone, all standing on a plane.]{\includegraphics[width=.24\textwidth]{img/wiki_frame_buffer.png}}%
			\subfigure[Example of a z-buffer. The lighter a pixel, the further away it is from the cameras point of view.]{\includegraphics[width=.24\textwidth]{img/wiki_z_buffer.png}}%
			\subfigure[Example of a normal-buffer. The x, y and z components of a normal-vector correspond to an RGB color.]{\includegraphics[width=.24\textwidth]{img/wiki_normal_buffer.png}}%
			\caption{G-Buffers}
			\label{fig:deferred_shading}%
		\end{figure}

		Each geometry-buffer stores information of some sort for each individual pixel, meaning that they are all two-dimensional arrays using the dimensions of the screen. Note that there are more possible buffers to choose from (position-buffer, stencil-buffer, ...), but these three are the most essential:
		\subsubsection{Albedo-buffer}%
			Unshaded color-values of frontmost fragments are stored in the albedo-buffer.% 
		\subsubsection{Z-buffer}
			%TODO maybe explain screenspace and worldspace
			The z-buffer stores depth values of fragments. These are needed to determine which surfaces are closest and visible to the camera. If two different fragments have the same x and y coordinates in screenspace, then the fragment with the smaller z-value is supposed to be in front of the other. This buffer is also used for screenspace techniques like SSAO\footnote{Screen space ambient occlusion} and depth-peeling. 
	...
	\subsection{Geometry pass}
		collect g-buffars TODO
