...
\section{Geometry-buffer (g-buffer)}
	...
	\subsection{Computing local illumination using g-buffers (deferred shading)}
		After having collected all our g-buffers we can now work on illumination. To do this we need to define some light sources. The following are possible light-sources that we can use:
...
\section{Deep g-buffer}
	...
	\subsection{Generating a 2-layer deep g-buffer}
		A 2-layer deep g-buffer can be efficiently generated in a single pass by ...
